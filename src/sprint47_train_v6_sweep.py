"""
sprint47_train_v6_sweep.py -- Sprint 47: V6 Robust Anchor LSTM Sweep
=====================================================================

Purpose
-------
Test whether the V5 -20C Anchor Collapse is an optimization issue by adding an
explicit weighted anchor loss, without changing the V5 HardCoulombLSTM
parameter architecture.

Safety
------
This script trains only Scenario A and writes only V6 alpha-sweep artifacts:
  - outputs/models/best_model_v6_alpha_{alpha}.pt
  - logs/training_log_v6_alpha_{alpha}.csv
  - logs/sprint47_v6_sweep_results.json

It does not read, modify, or overwrite V3/V4/V5 checkpoints.

Usage
-----
python src/sprint47_train_v6_sweep.py
"""
...
PATIENCE = 10
SCENARIO = "scenario_A"
ARTIFACT_PREFIX = "v6_alpha"
SUMMARY_PATH = os.path.join(LOG_DIR, "sprint47_v6_sweep_results.json")


class ExposedHardCoulombLSTM(HardCoulombLSTM):
    """V5 HardCoulombLSTM with the same parameters but exposed internals.

    The original V5 architecture is unchanged; this subclass only overrides the
    forward method so the training loop can apply anchor loss directly.
    """
    ...
    def _apply_hard_coulomb(self, delta_soc_raw: torch.Tensor,
        ...
        coulomb_limit = torch.abs(I_t) * self.hard_constraint.gamma * self.hard_constraint.safety_factor

        discharge_mask = I_t < -self.hard_constraint.threshold
        charge_mask = I_t > self.hard_constraint.threshold
...
def build_model(seed_offset: int = 0) -> ExposedHardCoulombLSTM:
    set_seed(RANDOM_SEED + seed_offset)
    return ExposedHardCoulombLSTM(
...
def artifact_paths(alpha: float) -> Dict[str, str]:
    tag = alpha_tag(alpha)
    return {
        "best_model": os.path.join(OUTPUT_MOD, f"best_model_v6_alpha_{tag}.pt"),
        "train_log": os.path.join(LOG_DIR, f"training_log_v6_alpha_{tag}.csv"),
    }
...
def compute_pvr(y_pred: np.ndarray, current_values: np.ndarray):
    ...
    discharge_mask = current_values[:, 1:] < CURRENT_THRESHOLD * -1.0
...
def train_one_epoch(model, loader, criterion, optimizer, alpha: float, epoch: int, total_epochs: int):
    ...
    pbar = tqdm(
        loader,
        desc=f"Alpha {alpha_tag(alpha)} | Epoch {epoch}/{total_epochs} Train",
        leave=False,
        dynamic_ncols=True,
    )
...
def validate(model, loader, criterion, alpha: float, epoch: int, total_epochs: int):
    ...
    pbar = tqdm(
        loader,
        desc=f"Alpha {alpha_tag(alpha)} | Epoch {epoch}/{total_epochs} Val",
        leave=False,
        dynamic_ncols=True,
    )
    with torch.no_grad():
...
def evaluate_test(model, test_loader, alpha: float) -> Dict:
    ...
    with torch.no_grad():
        for X_b, y_b, I_b in tqdm(
            test_loader,
            desc=f"Alpha {alpha_tag(alpha)} | Test Eval",
            leave=False,
            dynamic_ncols=True,
        ):
            X_b = X_b.to(DEVICE)
...
def append_summary(result: Dict, path: str) -> None:
    ...
    payload = {
        "sprint": 47,
        "experiment": "V6 Robust Anchor LSTM alpha sweep",
        "scenario": SCENARIO,
...
def train_alpha(alpha: float, train_loader, val_loader, test_loader, epochs: int,
    ...
    log_print(f"\n{'=' * 78}")
    log_print(f"  V6 Alpha Sweep | alpha={alpha_tag(alpha)} | Scenario A")
    log_print(f"{'=' * 78}")
    log_print(f"  Fresh model initialized | params={count_parameters(model):,}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        log_print(
            f"  Alpha {alpha_tag(alpha)} | Epoch {epoch:3d}/{epochs} | "
        ...
        if patience_counter >= PATIENCE:
            log_print(f"\n  Early stopping alpha={alpha_tag(alpha)} at epoch {epoch}")
            break
    ...
    log_print(f"\n  Alpha {alpha_tag(alpha)} test metrics:")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Sprint 47 V6 alpha sweep for HardCoulombLSTM anchor loss.")
...
def main() -> None:
    ...
    log_print("=" * 78)
    log_print("  Sprint 47: V6 Robust Anchor LSTM Alpha Sweep")
    log_print("  V5 HardCoulombLSTM architecture + weighted anchor objective")
    log_print("=" * 78)
    log_print(f"  Device: {DEVICE} | Batch: {args.batch_size} | Epochs/alpha: {args.epochs}")
    log_print(f"  Alpha values: {ALPHA_VALUES}")
    log_print(f"  Artifact namespace: _v6_alpha_{{val}}_")
    log_print("  V5 artifacts are not read or overwritten.")
    ...
    for run_idx, alpha in enumerate(ALPHA_VALUES):
        try:
            ...
            if "out of memory" in str(exc).lower() and DEVICE.type == "cuda":
                torch.cuda.empty_cache()
                raise RuntimeError(
                    "CUDA OOM during alpha sweep. Re-run with a smaller --batch-size."
                ) from exc
            raise
        all_results.append(result)

    log_print(f"\n{'=' * 78}")
    log_print("  Sprint 47 V6 Alpha Sweep Complete")
    log_print(f"{'=' * 78}")

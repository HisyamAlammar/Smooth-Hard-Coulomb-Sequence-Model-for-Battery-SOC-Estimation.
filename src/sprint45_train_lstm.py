"""
sprint45_train_lstm.py — Sprint 45: Hybrid Physics-LSTM Ablation
=================================================================
Trains HybridPhysicsLSTM (v4) on the SAME v3 preprocessed data.
All output artifacts use '_v4_lstm_' naming to protect existing V3 files.

Usage: python src/sprint45_train_lstm.py [--resume] [--eval-only]
"""
...
from model_v4_lstm import HybridPhysicsLSTM, count_parameters
...
def train_scenario(scenario_name: str, batch_size: int = BATCH_SIZE,
                   resume: bool = False):
    log_print(f"\n{'='*65}")
    log_print(f"  TRAINING v4-LSTM: {scenario_name}")
    log_print(f"  Hybrid Physics-LSTM | Hard Constraint | Pure MSE")
    log_print(f"{'='*65}")
    ...
    # SAFE naming — '_v4_lstm_' prefix to avoid overwriting V3
    log_path    = os.path.join(LOG_DIR,      f"training_log_v4_lstm_{scenario_name}.csv")
    best_path   = os.path.join(OUTPUT_MOD,   f"hybrid_v4_lstm_{scenario_name}.pt")
    latest_path = os.path.join(OUTPUT_MOD,   f"hybrid_v4_lstm_{scenario_name}_latest.pt")
    ...
    for epoch in range(start_epoch, EPOCHS + 1):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({
                ...
                "config": {
                    "version": "v4_lstm", "num_inputs": NUM_INPUTS,
...
def plot_soc(y_true, y_pred, scenario_name, save_path, n_points=5000):
    ...
    plt.rcParams.update({
        ...
        "axes.grid": True, "grid.alpha": 0.25, "lines.linewidth": 0.9,
    })
    ...
    fig.suptitle(f"Hybrid Physics-LSTM (v4) -- {scenario_name}",
...
def run_eval_and_plot(scenario_name: str, device):
    log_print(f"\n{'='*65}")
    log_print(f"  EVAL-ONLY v4-LSTM: {scenario_name}")
    log_print(f"{'='*65}")
    ...
    # Plot with SAFE filename
    os.makedirs(OUTPUT_FIG, exist_ok=True)
    plot_soc(yt, yp, scenario_name.replace('_', ' ').title(),
             os.path.join(OUTPUT_FIG, f"v4_lstm_soc_{scenario_name}.png"))
...
# =====================================================================
def main():
    parser = argparse.ArgumentParser(description="Sprint 45: Hybrid Physics-LSTM (v4)")
    parser.add_argument('--resume', action='store_true')
    parser.add_argument('--eval-only', action='store_true')
    args = parser.parse_args()

    log_print("=" * 65)
    log_print("  Sprint 45: Hybrid Physics-LSTM (v4) Ablation")
    log_print("  LSTM backbone + Hard Constraint | Pure MSE | PVR=0% guaranteed")
    if args.eval_only:
    ...
    if args.eval_only:
        ...
        log_print(f"\n{'='*65}")
        log_print(f"  v4-LSTM FINAL COMPARISON (eval-only)")
        log_print(f"{'='*65}")
    ...
    for scenario in ["scenario_A", "scenario_B"]:
        ...
        plot_soc(yt, yp, scenario.replace('_', ' ').title(),
                 os.path.join(OUTPUT_FIG, f"v4_lstm_soc_{scenario}.png"))

        del model, test_loader
        if DEVICE.type == "cuda": torch.cuda.empty_cache()

    # Save results JSON with SAFE filename
    results_path = os.path.join(LOG_DIR, "sprint45_results_v4_lstm.json")
    ...
    # Final comparison
    log_print(f"\n{'='*65}")
    log_print(f"  v4-LSTM FINAL COMPARISON")
    log_print(f"{'='*65}")
    log_print(f"  {'Metric':<18} {'Scenario A':<18} {'Scenario B':<18}")
    log_print(f"  {'-'*54}")
    ...
    log_print(f"\n  All v4-LSTM artifacts saved (V3 files untouched).")
